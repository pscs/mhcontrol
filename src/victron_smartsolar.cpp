...
void VictronSmartSolar::askForInitialValues() {
	...
	//Max MTU is 23, so can't send more than 3 requests (18 bytes) in one Write

    const uint8_t v1[] = {0x05, 0x03, 0x81, 0x19, 0xed, 0x8d, //v1
        0x05, 0x03, 0x81, 0x19, 0xed, 0x8f, //i1
        0x05, 0x03, 0x81, 0x19, 0x02, 0x01}; //status
    const uint8_t v2[] = {0x05, 0x03, 0x81, 0x19, 0xed, 0xbc, //panel power
        0x05, 0x03, 0x81, 0x19, 0xed, 0xbb, //panel voltage
        0x05, 0x03, 0x81, 0x19, 0xed, 0xdb}; //temp

	/*	    const uint8_t v3[] = {0x05, 0x03, 0x81, 0x19, 0xed, 0x8d, //v1
        0x05, 0x03, 0x81, 0x19, 0xed, 0x8f, //i1
        0x05, 0x03, 0x81, 0x19, 0x02, 0x01,
		0x05, 0x03, 0x81, 0x19, 0xed, 0xbc, //panel power
        0x05, 0x03, 0x81, 0x19, 0xed, 0xbb, //panel voltage
        0x05, 0x03, 0x81, 0x19, 0xed, 0xdb}; //temp */
    pChar3->writeValue((uint8_t*)v1, sizeof(v1), false);
    pChar3->writeValue((uint8_t*)v2, sizeof(v2), false);
